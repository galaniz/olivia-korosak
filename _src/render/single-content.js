...
const singleContent = async ({
  ...
  const allLink = `
    <div class="e-underline pt-4xs">
      <div class="inline-flex align-center">
        <span class="flex w-2xs h-2xs l-svg">
          ${caretSvg('left')}
        </span>
        <span>
          <a href="${allPermalink}" class="text-m wt-medium lead-base" data-rich>
            All ${plural}
          </a>
        </span>
      </div>
  ...
  if (contentType === 'track') {
    ...
    if (item?.fields?.audio && item?.fields?.audioDuration) {
      ...
      details = `
        <div class="pl-4xl-l mb-2xs-all mb-s-all-m">
          ...
          <dl class="flex col row-s wrap gap-s gap-l-m text-l muted num-normal lead-base e-underline-reverse">
            ${detailsItems.map(d => {
              return `
                <div>
                  <dt class="lead-base mb-5xs mb-4xs-m">${d.title}</dt>
                  <dd>${d.desc}</dd>
                </div>
              `
            }).join('')}
    ...
    if (similar) {
      similar = `
        <div class="pl-4xl-l mb-2xs-all mb-s-all-m">
          <h2 class="heading-m">${similarTitle}</h2>
          ${similar}
          ${allLink}
        </div>
      `

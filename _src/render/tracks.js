  ...
  for (let i = 0; i < items.length; i++) {
    ...
    const cells = [{
      ...
      output: `
        <div class="flex gap-2xs gap-s-m align-center relative">
          <div>
            <button type="button" id="b-${id}" class="o-play w-m h-m l-svg sharp bg-background-light b-radius-100-pc" aria-label="Play ${title}" data-state="play">
              ${controlSvg('play')}
              ${controlSvg('pause')}
            </button>
          </div>
          <div class="text-l wt-medium clamp-1 e-underline-reverse outline-tight">
    ...
    if (includeProjects) {
      ...
      cells.push({
        ...
        headers: 'projects',
        output: `
          <span class="text-m-flex muted lead-base clamp-2 relative e-underline-reverse e-underline-thin outline-tight">
            ${projects}
          </span>
        `
      })
    ...
    cells.push({
      ...
      output: `
        <button class="w-s h-s flex align-center justify-center ml-auto relative" type="button" aria-label="Toggle ${title} details" id="${triggerId}" aria-controls="${detailsId}" aria-expanded="false">
          <span class="flex w-xs h-xs l-svg sharp o-collapsible-icon e-transition">
            ${caretSvg('down')}
          </span>
        </button>
      `
      ...
      <tr data-mobile>
        <td headers="details" colspan="2">
          <div class="o-track-bg-b relative before">
            <div class="o-collapsible relative z-index-1" id="${detailsId}" data-trigger="${triggerId}" data-accordion="${accordionId}">
              <div id="${collapsibleId}" class="o-collapsible__main e-transition outline-tight">
                <dl class="text-m-flex num-normal muted lead-base e-underline-reverse m-0-last pb-2xs pb-s-m">
                  ${detailsItems.map(d => {
                    return `

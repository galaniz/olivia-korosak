...
const _getContent = ({
  ...
  content.forEach(c => {
    ...
    if (nodeType === 'text' && value) {
      ...
      _output += cardLink ? `<a class="before" href="${cardLink}" data-rich>${linkText}</a>` : val
    }
...
const richText = (args = {}) => {
  ...
  if (tag === 'table') {
    output = `
      <div class="overflow-hidden" data-rich-table>
        <div class="overflow-x-auto w-full b-all o-overflow">
          ${output}
        </div>
      </div>
    `

...
/**
 * Gradients for page or card.
 *
 * @param {string} hex
 * @param {'card'|'page'} [type='card']
...
const getGradient = (hex: string, type: 'card' | 'page' = 'card', full: boolean = true): string => {
  ...
  addStyle('utils/gradient/gradient')

  return /* html */`
    <div
      class="gradients w-full h-full absolute top-0 left-0 overflow-hidden z--1 before${full ? ' after' : ''}"
      style="${styles};--ok-bg-between:${colorBetween}"
    ></div>
  `
}
